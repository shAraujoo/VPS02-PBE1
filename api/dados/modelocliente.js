...
const getClientById = (id, callback) => {
    connect.query('SELECT * FROM clientes WHERE idcliente = ?', [id], (err, results) => {
        if (err) return callback(err, null);
...
const updateCliente = (id, data, callback) => {
    connect.query('UPDATE clientes SET ? WHERE idcliente = ?', [data, id], (err, results) => {
        if (err) return callback(err, null);
...
const deleteCliente = (id, callback) => {
    connect.query('DELETE FROM clientes WHERE idcliente = ?', [id], (err, results) => {
        if (err) return callback(err, null);
